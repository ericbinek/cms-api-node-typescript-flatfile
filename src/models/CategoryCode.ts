...
const COLLECTION_FILE = "category-codes.json";
...
const REQUIRED_FIELDS: Set<string> = new Set(["name","codeValue","inCodeSet"]);
const SEARCHABLE_FIELDS: Set<string> = new Set(["name","description","codeValue"]);
const SORTABLE_FIELDS: Set<string> = new Set(["dateCreated", "dateModified", ...["name","description","codeValue","url"]]);

const SYSTEM_FIELDS: Set<string> = new Set(['id', 'dateCreated', 'dateModified', '@context', '@type']);

const REF_COLLECTIONS: Record<string, string> = {"CategoryCodeSet":"category-code-sets.json"};

// Properties whose combined value must be unique across the collection. Empty
// when the entity allows duplicates.
const UNIQUE_KEY: readonly string[] = ["codeValue","inCodeSet"];
...
export async function findAll(options: FindAllOptions = {}): Promise<ListResult<CategoryCode>> {
  ...
  const items = results.slice(offset, offset + limit) as unknown as CategoryCode[];
  return { items, total };
}

export async function findById(id: string): Promise<CategoryCode | null> {
...
export async function embedRefs(item: CategoryCode): Promise<Record<string, unknown>> {
...
export function update(id: string, rawData: unknown): Promise<CategoryCode | null> {
  return withLock(async () => {
    const items = await readCollection<CategoryCode>(COLLECTION_FILE);
    const normalized = normalizeUUID(id);
...
export function etagOf(item: CategoryCode): string {
  return etagFor(item);
}
