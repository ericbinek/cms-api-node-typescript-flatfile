...
const COLLECTION_FILE = "image-objects.json";
const TYPE_NAME = 'ImageObject';
...
const FIELDS: Record<string, FieldSpec> = {
  ...
  "license": { kind: 'scalar', type: "URL", cardinality: "one", maxLength: 2048 },
};
const FIELD_NAMES: Set<string> = new Set(Object.keys(FIELDS));
const REQUIRED_FIELDS: Set<string> = new Set(["contentUrl"]);
const SEARCHABLE_FIELDS: Set<string> = new Set(["name","caption","description","encodingFormat"]);
const SORTABLE_FIELDS: Set<string> = new Set(["dateCreated", "dateModified", ...["name","caption","description","contentUrl","encodingFormat","uploadDate","license"]]);

const SYSTEM_FIELDS: Set<string> = new Set(['id', 'dateCreated', 'dateModified', '@context', '@type']);

const REF_COLLECTIONS: Record<string, string> = {"Person":"persons.json"};
...
export async function findAll(options: FindAllOptions = {}): Promise<ListResult<ImageObject>> {
  ...
  const items = results.slice(offset, offset + limit) as unknown as ImageObject[];
  return { items, total };
}

export async function findById(id: string): Promise<ImageObject | null> {
  if (!isValidUUID(id)) return null;
  const items = await readCollection<ImageObject>(COLLECTION_FILE);
...
export async function embedRefs(item: ImageObject): Promise<Record<string, unknown>> {
...
export function create(rawData: unknown): Promise<ImageObject> {
...
export function update(id: string, rawData: unknown): Promise<ImageObject | null> {
  return withLock(async () => {
    const items = await readCollection<ImageObject>(COLLECTION_FILE);
    const normalized = normalizeUUID(id);
...
export function etagOf(item: ImageObject): string {
  return etagFor(item);
}
