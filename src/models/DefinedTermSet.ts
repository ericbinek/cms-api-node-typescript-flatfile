...
const COLLECTION_FILE = "defined-term-sets.json";
const TYPE_NAME = 'DefinedTermSet';
...
export async function findAll(options: FindAllOptions = {}): Promise<ListResult<DefinedTermSet>> {
  ...
  const items = results.slice(offset, offset + limit) as unknown as DefinedTermSet[];
  return { items, total };
}

export async function findById(id: string): Promise<DefinedTermSet | null> {
  if (!isValidUUID(id)) return null;
  const items = await readCollection<DefinedTermSet>(COLLECTION_FILE);
...
export async function embedRefs(item: DefinedTermSet): Promise<Record<string, unknown>> {
...
export function create(rawData: unknown): Promise<DefinedTermSet> {
...
export function update(id: string, rawData: unknown): Promise<DefinedTermSet | null> {
  return withLock(async () => {
    const items = await readCollection<DefinedTermSet>(COLLECTION_FILE);
    const normalized = normalizeUUID(id);
...
export function etagOf(item: DefinedTermSet): string {
  return etagFor(item);
}
