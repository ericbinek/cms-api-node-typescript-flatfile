...
export async function findAll(options: FindAllOptions = {}): Promise<ListResult<WebSite>> {
  ...
  const items = results.slice(offset, offset + limit) as unknown as WebSite[];
  return { items, total };
}

export async function findById(id: string): Promise<WebSite | null> {
  if (!isValidUUID(id)) return null;
  const items = await readCollection<WebSite>(COLLECTION_FILE);
...
export async function embedRefs(item: WebSite): Promise<Record<string, unknown>> {
...
export function create(rawData: unknown): Promise<WebSite> {
...
export function update(id: string, rawData: unknown): Promise<WebSite | null> {
  return withLock(async () => {
    const items = await readCollection<WebSite>(COLLECTION_FILE);
    const normalized = normalizeUUID(id);
...
export function etagOf(item: WebSite): string {
  return etagFor(item);
}
