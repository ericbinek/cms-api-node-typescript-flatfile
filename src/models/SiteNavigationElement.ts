...
const SORTABLE_FIELDS: Set<string> = new Set(["dateCreated", "dateModified", ...["name","url","description","position"]]);
...
export async function findAll(options: FindAllOptions = {}): Promise<ListResult<SiteNavigationElement>> {
  ...
  const items = results.slice(offset, offset + limit) as unknown as SiteNavigationElement[];
  return { items, total };
}

export async function findById(id: string): Promise<SiteNavigationElement | null> {
...
export async function embedRefs(item: SiteNavigationElement): Promise<Record<string, unknown>> {
...
export function update(id: string, rawData: unknown): Promise<SiteNavigationElement | null> {
  return withLock(async () => {
    const items = await readCollection<SiteNavigationElement>(COLLECTION_FILE);
    const normalized = normalizeUUID(id);
...
export function etagOf(item: SiteNavigationElement): string {
  return etagFor(item);
}
