...
const COLLECTION_FILE = "video-objects.json";
...
const SEARCHABLE_FIELDS: Set<string> = new Set(["name","description","encodingFormat","videoQuality","transcript","caption"]);
const SORTABLE_FIELDS: Set<string> = new Set(["dateCreated", "dateModified", ...["name","description","contentUrl","embedUrl","encodingFormat","duration","videoQuality","transcript","caption","uploadDate"]]);
...
export async function findAll(options: FindAllOptions = {}): Promise<ListResult<VideoObject>> {
  ...
  const items = results.slice(offset, offset + limit) as unknown as VideoObject[];
  return { items, total };
}

export async function findById(id: string): Promise<VideoObject | null> {
...
export async function embedRefs(item: VideoObject): Promise<Record<string, unknown>> {
...
export function update(id: string, rawData: unknown): Promise<VideoObject | null> {
  return withLock(async () => {
    const items = await readCollection<VideoObject>(COLLECTION_FILE);
    const normalized = normalizeUUID(id);
...
export function etagOf(item: VideoObject): string {
  return etagFor(item);
}
