...
const FIELDS: Record<string, FieldSpec> = {
  ...
  "givenName": { kind: 'scalar', type: "Text", cardinality: "one" },
  "familyName": { kind: 'scalar', type: "Text", cardinality: "one" },
  "alternateName": { kind: 'scalar', type: "Text", cardinality: "one" },
...
const SEARCHABLE_FIELDS: Set<string> = new Set(["name","givenName","familyName","alternateName","email","description","jobTitle"]);
const SORTABLE_FIELDS: Set<string> = new Set(["dateCreated", "dateModified", ...["name","givenName","familyName","alternateName","email","url","description","jobTitle"]]);
...
export async function findAll(options: FindAllOptions = {}): Promise<ListResult<Person>> {
  ...
  const items = results.slice(offset, offset + limit) as unknown as Person[];
  return { items, total };
}

export async function findById(id: string): Promise<Person | null> {
  if (!isValidUUID(id)) return null;
  const items = await readCollection<Person>(COLLECTION_FILE);
...
export async function embedRefs(item: Person): Promise<Record<string, unknown>> {
...
export function create(rawData: unknown): Promise<Person> {
...
export function update(id: string, rawData: unknown): Promise<Person | null> {
  return withLock(async () => {
    const items = await readCollection<Person>(COLLECTION_FILE);
    const normalized = normalizeUUID(id);
...
export function etagOf(item: Person): string {
  return etagFor(item);
}
