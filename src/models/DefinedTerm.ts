...
const COLLECTION_FILE = "defined-terms.json";
...
const REQUIRED_FIELDS: Set<string> = new Set(["name","termCode","inDefinedTermSet"]);
const SEARCHABLE_FIELDS: Set<string> = new Set(["name","description","termCode"]);
const SORTABLE_FIELDS: Set<string> = new Set(["dateCreated", "dateModified", ...["name","description","termCode","url"]]);

const SYSTEM_FIELDS: Set<string> = new Set(['id', 'dateCreated', 'dateModified', '@context', '@type']);

const REF_COLLECTIONS: Record<string, string> = {"DefinedTermSet":"defined-term-sets.json"};
...
export async function findAll(options: FindAllOptions = {}): Promise<ListResult<DefinedTerm>> {
  ...
  const items = results.slice(offset, offset + limit) as unknown as DefinedTerm[];
  return { items, total };
}

export async function findById(id: string): Promise<DefinedTerm | null> {
  if (!isValidUUID(id)) return null;
  const items = await readCollection<DefinedTerm>(COLLECTION_FILE);
...
export async function embedRefs(item: DefinedTerm): Promise<Record<string, unknown>> {
...
export function create(rawData: unknown): Promise<DefinedTerm> {
...
export function update(id: string, rawData: unknown): Promise<DefinedTerm | null> {
  return withLock(async () => {
    const items = await readCollection<DefinedTerm>(COLLECTION_FILE);
    const normalized = normalizeUUID(id);
...
export function etagOf(item: DefinedTerm): string {
  return etagFor(item);
}
