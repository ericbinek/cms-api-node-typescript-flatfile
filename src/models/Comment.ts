...
export interface Comment {
  ...
  creativeWorkStatus?: "Pending" | "Approved" | "Spam" | "Trash";
}

const COLLECTION_FILE = "comments.json";
...
const FIELDS: Record<string, FieldSpec> = {
  ...
  "upvoteCount": { kind: 'scalar', type: "Integer", cardinality: "one" },
  "downvoteCount": { kind: 'scalar', type: "Integer", cardinality: "one" },
  "creativeWorkStatus": { kind: 'enum', values: ["Pending","Approved","Spam","Trash"], cardinality: "one" },
};
const FIELD_NAMES: Set<string> = new Set(Object.keys(FIELDS));
const REQUIRED_FIELDS: Set<string> = new Set(["text","author","about"]);
const SEARCHABLE_FIELDS: Set<string> = new Set(["text"]);
const SORTABLE_FIELDS: Set<string> = new Set(["dateCreated", "dateModified", ...["text","dateCreated","dateModified","upvoteCount","downvoteCount","creativeWorkStatus"]]);

const SYSTEM_FIELDS: Set<string> = new Set(['id', 'dateCreated', 'dateModified', '@context', '@type']);

const REF_COLLECTIONS: Record<string, string> = {"Person":"persons.json","BlogPosting":"blog-postings.json","Comment":"comments.json"};
...
export async function findAll(options: FindAllOptions = {}): Promise<ListResult<Comment>> {
  ...
  const items = results.slice(offset, offset + limit) as unknown as Comment[];
  return { items, total };
}
...
export async function embedRefs(item: Comment): Promise<Record<string, unknown>> {
...
export function update(id: string, rawData: unknown): Promise<Comment | null> {
  return withLock(async () => {
    const items = await readCollection<Comment>(COLLECTION_FILE);
    const normalized = normalizeUUID(id);
...
export function etagOf(item: Comment): string {
  return etagFor(item);
}
