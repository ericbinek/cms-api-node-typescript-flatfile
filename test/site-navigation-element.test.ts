...
test(`${ENTITY}: sort by ${"name"} accepted`, async () => {
  const r = await authedFetch(`${server.baseUrl}${BASE}?sort=${"name"}&order=asc`);
  assert.equal(r.status, 200);
});
...
test(`${ENTITY}: sort by numeric field "position" orders numerically with missing last`, async () => {
  const ids: string[] = [];
  ...
  const res = await authedFetch(`${server.baseUrl}${BASE}?sort=position&order=asc&limit=100`);
  assert.equal(res.status, 200);
  const body = await jsonOf(res);
  const idSet = new Set(ids);
  const mine = body.items.filter((i: any) => idSet.has(i.id));
  const presentVals = mine
    .filter((i: any) => typeof i["position"] === 'number')
...
test(`${ENTITY}: filter on text field "name" returns matches`, async () => {
  ...
  const r = await authedFetch(`${server.baseUrl}${BASE}?name=${encodeURIComponent(needle)}`);
  const body = await jsonOf(r);
...
test(`${ENTITY}: GET by id embeds "isPartOf" as an object; list stays flat`, async () => {
  ...
  const embedded = got["isPartOf"];
  assert.equal(typeof embedded, 'object');
  assert.equal(embedded['@type'], "SiteNavigationElement");
  ...
  assert.equal(typeof inList["isPartOf"], 'string');
});

test(`${ENTITY}: GET by id leaves an unresolvable "isPartOf" ref as its UUID`, async () => {
  const DANGLING = '00000000-0000-0000-0000-000000000000';
  const payload = await buildPayload(server.baseUrl, ENTITY, { partial: true });
  payload["isPartOf"] = DANGLING;
  const created = await jsonOf(await postEntity(server.baseUrl, ENTITY, payload));
  const got = await jsonOf(await authedFetch(`${server.baseUrl}${BASE}/${created.id}`));
  assert.equal(got["isPartOf"], DANGLING);
});
