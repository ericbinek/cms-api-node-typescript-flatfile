...
test(`${ENTITY}: GET by id embeds "inDefinedTermSet" as an object; list stays flat`, async () => {
  ...
  const refId = created["inDefinedTermSet"];
  ...
  const embedded = got["inDefinedTermSet"];
  assert.equal(typeof embedded, 'object');
  assert.equal(embedded['@type'], "DefinedTermSet");
  ...
  assert.equal(typeof inList["inDefinedTermSet"], 'string');
});

test(`${ENTITY}: GET by id leaves an unresolvable "inDefinedTermSet" ref as its UUID`, async () => {
