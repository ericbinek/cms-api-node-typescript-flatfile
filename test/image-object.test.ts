...
test(`${ENTITY}: GET by id embeds "creator" as an object; list stays flat`, async () => {
  ...
  assert.equal(typeof inList["creator"], 'string');
});

test(`${ENTITY}: GET by id leaves an unresolvable "creator" ref as its UUID`, async () => {
  const DANGLING = '00000000-0000-0000-0000-000000000000';
  const payload = await buildPayload(server.baseUrl, ENTITY, { partial: true });
  payload["creator"] = DANGLING;
  const created = await jsonOf(await postEntity(server.baseUrl, ENTITY, payload));
  const got = await jsonOf(await authedFetch(`${server.baseUrl}${BASE}/${created.id}`));
  assert.equal(got["creator"], DANGLING);
});
