...
test(`${ENTITY}: GET by id embeds "inCodeSet" as an object; list stays flat`, async () => {
  ...
  const embedded = got["inCodeSet"];
  assert.equal(typeof embedded, 'object');
  assert.equal(embedded['@type'], "CategoryCodeSet");
  ...
  assert.equal(typeof inList["inCodeSet"], 'string');
});

test(`${ENTITY}: GET by id leaves an unresolvable "inCodeSet" ref as its UUID`, async () => {
  const DANGLING = '00000000-0000-0000-0000-000000000000';
  const payload = await buildPayload(server.baseUrl, ENTITY, { partial: true });
  payload["inCodeSet"] = DANGLING;
  const created = await jsonOf(await postEntity(server.baseUrl, ENTITY, payload));
  const got = await jsonOf(await authedFetch(`${server.baseUrl}${BASE}/${created.id}`));
  assert.equal(got["inCodeSet"], DANGLING);
});
